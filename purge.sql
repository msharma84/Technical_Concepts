...
then
   INSERT INTO queue_history 
    ...
    VALUES(new.column_1_data,
         ...
         new.column_3_data); 
End If;
totalcount:= (select count(*) from pms_pcc_queue_history where column_1=new.column_1);
if(totalcount>10) 
then 
   delete from queue_history where id in
    (select id from queue_history where column_1=new.column_1 order by id limit 1);
    delete from pms_pcc_queue where id in (select id from queue where status in ('processed','failed'));
end if;
